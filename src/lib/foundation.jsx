...
export const TaskModal = ({ onClose, onSave, t, defaultDate, employees, categories = categoryList, templates = TASK_TEMPLATES }) => { const [title,setTitle]=useState(""); const [assignee,setA]=useState(employees[0]?.name || ""); const [category,setC]=useState(categories[0]); const [priority,setP]=useState("moyenne"); const [dueDate,setD]=useState(defaultDate||TODAY); const [addToTemplates,setAddToTemplates]=useState(false); const [tmplCreneau,setTmplCreneau]=useState("service"); const sel={ width:"100%", padding:"10px 12px", borderRadius:8, border:`1px solid ${t.border}`, fontSize:14, fontFamily:F, background:t.surface, color:t.text, outline:"none", cursor:"pointer" }; return (<div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000 }} onClick={onClose}><div style={{ background:t.surface, borderRadius:16, padding:28, width:460, maxWidth:"92vw", boxShadow:"0 20px 60px rgba(0,0,0,0.15)" }} onClick={e=>e.stopPropagation()}><div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}><h2 style={{ fontSize:18, fontWeight:700, margin:0, fontFamily:F }}>Nouvelle tâche</h2><button onClick={onClose} style={{ background:"none", border:"none", cursor:"pointer", color:t.textMuted, padding:4 }}>{I.x}</button></div><div style={{ display:"flex", flexDirection:"column", gap:16 }}><div><label style={{ fontSize:13, fontWeight:600, color:t.textMuted, marginBottom:6, display:"block", fontFamily:F }}>Piocher dans la liste habituelle (optionnel)</label><select value="" onChange={e=>{ const v=e.target.value; if(v==="")return; const tmpl=templates[+v]; if(tmpl){ setTitle(tmpl.title); setC(tmpl.category); setP(tmpl.priority); } }} style={sel}><option value="">— Choisir une tâche habituelle —</option>{[{key:"ouverture",label:"Ouverture"},{key:"service",label:"Service"},{key:"fermeture",label:"Fermeture"}].map(c=>(<optgroup key={c.key} label={c.label}>{templates.map((tmpl,i)=>({tmpl,i})).filter(x=>x.tmpl.creneau===c.key).map(x=>(<option key={x.i} value={x.i}>{x.tmpl.title}</option>))}</optgroup>))}</select></div><div><label style={{ fontSize:13, fontWeight:600, color:t.textMuted, marginBottom:6, display:"block", fontFamily:F }}>Intitulé</label><input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Ex: Nettoyer les friteuses…" autoFocus style={sel} /></div><div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}><div><label style={{ fontSize:13, fontWeight:600, color:t.textMuted, marginBottom:6, display:"block", fontFamily:F }}>Assigné à</label><select value={assignee} onChange={e=>setA(e.target.value)} style={sel}>{employees.map(e=><option key={e.name}>{e.name}</option>)}</select></div><div><label style={{ fontSize:13, fontWeight:600, color:t.textMuted, marginBottom:6, display:"block", fontFamily:F }}>Date prévue</label><input type="date" value={dueDate} onChange={e=>setD(e.target.value)} style={sel} /></div></div><div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}><div><label style={{ fontSize:13, fontWeight:600, color:t.textMuted, marginBottom:6, display:"block", fontFamily:F }}>Catégorie</label><select value={category} onChange={e=>setC(e.target.value)} style={sel}>{categories.map(c=><option key={c}>{c}</option>)}</select></div><div><label style={{ fontSize:13, fontWeight:600, color:t.textMuted, marginBottom:8, display:"block", fontFamily:F }}>Priorité</label><div style={{ display:"flex", gap:6 }}>{priorityList.map(p=>(<button key={p} onClick={()=>setP(p)} style={{ flex:1, padding:"8px 0", borderRadius:8, fontSize:12, fontWeight:600, fontFamily:F, cursor:"pointer", textTransform:"capitalize", background:priority===p?(p==="haute"?t.danger:p==="moyenne"?t.warning:t.success):t.surfaceAlt, color:priority===p?"#fff":t.textMuted, border:priority===p?"none":`1px solid ${t.border}` }}>{p}</button>))}</div></div></div><div><label style={{ display:"flex", alignItems:"center", gap:8, fontSize:13, fontWeight:600, color:t.textMuted, fontFamily:F, cursor:"pointer" }}><input type="checkbox" checked={addToTemplates} onChange={e=>setAddToTemplates(e.target.checked)} style={{ accentColor:t.primary }} />Ajouter aux tâches habituelles</label>{addToTemplates&&<select value={tmplCreneau} onChange={e=>setTmplCreneau(e.target.value)} style={{ ...sel, marginTop:8 }}><option value="ouverture">🌅 Ouverture</option><option value="service">🍽️ Service</option><option value="fermeture">🌙 Fermeture</option></select>}</div></div><div style={{ display:"flex", gap:10, marginTop:24 }}><button onClick={onClose} style={{ flex:1, padding:"10px 0", borderRadius:8, border:`1px solid ${t.border}`, background:t.surface, color:t.textMuted, fontSize:14, fontWeight:600, cursor:"pointer", fontFamily:F }}>Annuler</button><button onClick={()=>{if(title.trim())onSave({title,assignee,category,priority,dueDate,addToTemplates,tmplCreneau});}} style={{ flex:1, padding:"10px 0", borderRadius:8, border:"none", background:title.trim()?t.primary:t.border, color:title.trim()?"#fff":t.textMuted, fontSize:14, fontWeight:600, cursor:title.trim()?"pointer":"default", fontFamily:F }}>Créer</button></div></div></div>); };
